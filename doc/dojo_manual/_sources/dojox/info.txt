...
module name, it is meant to be clear the module modifies something elsewhere in Dojo. For instance, ``dojox.fx.ext-dojo.NodeList`` adds :ref:`dojox.fx <dojox/fx>` functionality into :ref:`dojo.NodeList <dojo/NodeList>`, making it available from a :ref:`dojo.query <dojo/query>` call. 
The rational is: Because the hypen is illegal in JavaScript variables, you will never be able to directly call an ``ext-dojo`` method directly, and the act of :ref:`requiring <dojo/require>` it mixes the desired functionality.
