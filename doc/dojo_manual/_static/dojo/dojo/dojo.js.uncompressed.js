...
if(!dojo._hasResource["docs.widget.CodeViewer"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
...
dojo.declare("docs.widget.CodeViewer", [dijit._Widget, dijit._Templated],
{
	...
	postCreate: function(){
		// if this source has to be executed it will
		var node = dojo.query("pre", this.containerNode);
		
		var w = dojo.isIE ? "innerText" : "innerHTML";  
		var code = (node[0] ? node[0][w] : this.containerNode[w]);

		// if this source has to be viewable it will
		if (this.displayCode){
			var ret, tarea = dojo.doc.createElement('textarea');
	    	(dojo.isIE ? tarea.innerText = code : tarea.innerHTML = code);
			ret = tarea.value;
			
			// strange logic, when we need to determine the end of all loaded 
			// dojo.require statements:
			this.codeView.attr("content", ret);
			this.codeView.onLoadDeferred.addCallback(dojo.hitch(this, function(){ 
				// addOnLoad is fired, when all missing require statement are finished
				// even when postloading dojo modules
		...
		// if this source has to be sourcable it will
		if (this.sourceCode){
		...
		// implement the toggle switch nicely
...
if(!dojo._hasResource["docs.widget.CompoundViewer"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
...
dojo.declare("docs.widget.CompoundViewer",
	[dijit._Widget, dijit._Templated],
	{
	
	templateString:"<div dojoAttachPoint=\"outerNode\" class=\"compoundContainer\">\n\t<div class=\"themeSelector\" dojoAttachPoint=\"themeSelector\"></div>\n\t<div dojoAttachPoint=\"executedNode\"></div>\n\t<div dojoAttachPoint=\"contentNode\"></div>\n</div>\n",
	widgetsInTemplate: false,
		
	preview: {},
	createFullSource: true,
	
	themes: ["tundra", "nihilo", "soria"],
	defaultTheme: "nihilo",
	dojoPath: "../../../../",
	
	def: {
			javascript: {displayCode: false, sourceCode: true}, 
			css: {displayCode: false, sourceCode: true}, 
			html: {displayCode: true, sourceCode: true}
		},
	...
	postMixInProperties: function(){
		...
		var arrNodes = []; // arrays holding the codeviewer widgets and content
		var node = {};
		node.content = [];
		var index = 0;
		while (el){
			if (dojo.attr(el, "dojoType") != null) {
				...
				node = {"content": node.content, "label": label, "lang": el.lang, "code": el, "index": index};
				this.arrNodes[el.lang] = node;
				
				// reset node
				node = {"lang": "", "content": [], "code": ""};
				
				// remove dojotype so children widgets won't get parsed
				el.removeAttribute("dojoType");
				index++;
			}else{
				node.content.push(el);
			}		
			el = el.nextSibling;	
		}
	},
	...
	postCreate: function(){
		...
		// all other types of code can just be executed synchronosly
		for (node in this.arrNodes){
			this._buildCodeView(node);	
		};
		
		// if we have a html view we will display the result of our code here
		if (this.preview.code){
			var compoundElement = dojo.doc.createElement("div");
			dojo.addClass(compoundElement, "compoundElement");
			dojo.addClass(compoundElement, "compoundPreview");
			this.contentNode.appendChild(compoundElement);
			
			// code icon
			var icon = dojo.doc.createElement("div");
			dojo.addClass(icon, "iconpreview");
			compoundElement.appendChild(icon);

			var toggleNode = dojo.doc.createElement("div");

			var previewNode = dojo.doc.createElement("div");
			toggleNode.appendChild(previewNode);
			
			compoundElement.appendChild(toggleNode);
			previewNode.appendChild(this.preview.code);
			
			dojo.connect(icon, "onclick", dojo.hitch(this, function(node){
			...
			dojo.forEach(this.themes, function(theme){
				this[theme+"_themeSelector"] = new dijit.form.RadioButton({
						name: this.id+"_ThemeSelector",
						id: theme+"_"+this.id+"_themeSelector"});
						
				var label = dojo.doc.createElement("label");
				dojo.attr(label, "for", theme+"_"+this.id+"_themeSelector");
				label.innerHTML = theme;
				
				dojo.connect(this[theme+"_themeSelector"] , "onClick", this, function(e){
					(function(node, that){
						window.location = "?t="+node.id.split("_")[0];
					})(this[theme+"_themeSelector"], this);
				});
				
				this.themeSelector.appendChild(this[theme+"_themeSelector"].domNode);
		...
		// throw the results into the template
		dojo.forEach(this.arrViews, function(view){
			var compoundElement = dojo.doc.createElement("div");
			dojo.addClass(compoundElement, "compoundElement");
			this.contentNode.appendChild(compoundElement);
			...
			// code icon
			var icon = dojo.doc.createElement("div");
			dojo.addClass(icon, "icon"+view.type);
			compoundElement.appendChild(icon);
			
			var codeCompound = dojo.doc.createElement("div");
			compoundElement.appendChild(codeCompound);
			
			// hide code if set to by default
			if (this.hideCode){
				dojo.style(codeCompound, "display", "none");
			}
			dojo.connect(icon, "onclick", dojo.hitch(this, function(node){
				if (dojo.style(node, "display") == "none"){
					dojo.style(node, "display", "");
			...
			if (view.label){
				var label = dojo.doc.createElement("h3");
				label.innerHTML = view.label;
				dojo.addClass(label, "compoundLabel");
				codeCompound.appendChild(label);
			}
			if (view.content){
				var compoundText = dojo.doc.createElement("div");
				dojo.addClass(compoundText, "compoundText");
								
				dojo.forEach(view.content, function(node){
					compoundText.appendChild(node);
				}, this);
				codeCompound.appendChild(compoundText);
			}
			dojo.addClass(view.code.domNode, "compoundCode");
			codeCompound.appendChild(view.code.domNode);
		}, this);
		...
		// execute preview node last
		if (previewNode){
			var viewer = new docs.widget.CodeViewer({
					codeClass: "html", 
					displayCode: true, 
					sourceCode: false,
					sourceIsOpen: true}, 
	...
	setTheme: function(){
			...
			if(window.location.href.indexOf("?") > -1){
				...
				for(var i=0; i<ary.length; i++){
					...
					switch(key){
						case "t":
							dojo.attr(dojo.byId("themeCss"), "href", this.dojoPath+"dijit/themes/"+value+"/"+value+".css");
							dojo.body().className = value;
							
							this[value+"_themeSelector"].attr("checked", true);	
							break;
					}
				}
			}
	...
	_executeCode: function(codetype){
		var exDiv = dojo.doc.createElement("div");
		this.executedNode.appendChild(exDiv);
		
		// get source and make it viewable
		var ret, tarea = document.createElement('textarea');
		if (dojo.isIE){
			tarea.innerText = dojo.query("pre", this.arrNodes[codetype].code)[0].innerText;
		}else{
			tarea.innerHTML = dojo.query("pre", this.arrNodes[codetype].code)[0].innerHTML;
		}
		ret = tarea.value;
		this.arrExecuted[codetype] = new dojox.layout.ContentPane({"class": "codeHidden", renderStyles:"true"}, exDiv);
		// strange logic, when we need to determine the end of all loaded 
		// dojo.require statements:
		this.arrExecuted[codetype].attr("content", ret);
		this.arrExecuted[codetype].onLoadDeferred.addCallback(dojo.hitch(this, function(){ 
			// addOnLoad is fired, when all missing require statement are finished
			// even when postloading dojo modules
			dojo.addOnLoad(dojo.hitch(this, function(){
				dojo.parser.parse(this.arrExecuted[codetype].domNode);
			}));
		}));
	},
	
	_buildCodeView: function(codetype){
		// if type is html we'll add an extra view for the preview
		if (codetype == "html"){
			this.preview.code = this.arrNodes[codetype].code;
			this.def.html.displayCode = false;
		}
		
		// automaticallt create an indexed array to correct code order
		var node = dojo.doc.createElement("div");
		node.appendChild(dojo.clone(this.arrNodes[codetype].code));

		var a = this.arrViews[this.arrNodes[codetype].index] = [];
		a.type = codetype;
		a.label = this.arrNodes[codetype].label;
		a.content = this.arrNodes[codetype].content;

		a.code = new docs.widget.CodeViewer({
				codeClass: codetype, 
				displayCode: this.def[codetype].displayCode, 
				sourceCode: this.def[codetype].sourceCode,
				sourceIsOpen: true}, 
				node);
	},
	...
	_buildFullSource: function(){
		...
		if (this.arrNodes.css && this.arrNodes.css.code){
			var ret, tarea = dojo.doc.createElement('textarea');
	    	if (dojo.isIE){
				tarea.innerText = dojo.query("pre",this.arrNodes.css.code)[0].innerText;
	    	}else{
				tarea.innerHTML = dojo.query("pre",this.arrNodes.css.code)[0].innerHTML;
			}
			ret = tarea.value;
		...
		if (this.arrNodes.javascript && this.arrNodes.javascript.code){
			var ret, tarea = dojo.doc.createElement('textarea');
	    	if (dojo.isIE){
				tarea.innerText = dojo.query("pre",this.arrNodes.javascript.code)[0].innerText;
	    	}else{
				tarea.innerHTML = dojo.query("pre",this.arrNodes.javascript.code)[0].innerHTML;
			}
			ret = tarea.value;
		...
		if (this.arrNodes.html && this.arrNodes.html.code){
			var ret, tarea = dojo.doc.createElement('textarea');
	    	if (dojo.isIE){
				tarea.innerText = dojo.query("pre",this.arrNodes.html.code)[0].innerText;
	    	}else{
				tarea.innerHTML = dojo.query("pre",this.arrNodes.html.code)[0].innerHTML;
			}
			ret = tarea.value;
			messages.extraHTML = ret;
		}

		var fullSource = dojo.string.substitute(fullSource, messages);
		...
		// Label for the element
		var label = dojo.doc.createElement("h3");
		label.innerHTML = "Copy the full source";
		dojo.addClass(label, "compoundLabel");
		codeCompound.appendChild(label);
		...
		dojo.attr(textArea, "rows", 10);
		dojo.attr(textArea, "cols", 100);
			
		// set full source
		textArea.value = fullSource;
		compoundText.appendChild(textArea);
